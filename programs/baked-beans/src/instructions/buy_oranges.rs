...
pub fn handle(ctx: Context<BuyOranges>, sol_amount: u64) -> Result<()> {
    ...
    let mut oranges_bought =
        calculate_oranges_buy(&accts.global_state, sol_amount, accts.vault.lamports())?;

    let oranges_bought_fee = dev_fee(&accts.global_state, oranges_bought)?;
    oranges_bought = oranges_bought.checked_sub(oranges_bought_fee).unwrap();
    ...
    accts.user_state.claimed_oranges = accts
        .user_state
        .claimed_oranges
        .checked_add(oranges_bought)
        .unwrap();
    Ok(())
}
